...
import { sendEmail, sendLoopsTransactionalEmail } from "../emails/config.ts";
					...
					async sendVerificationRequest({ identifier, token, url }) {
						if (
							await sendLoopsTransactionalEmail({
								email: identifier,
								idempotencyKey: `mover-marketing-video-login-${identifier}-${token}`,
								dataVariables: {
									appName: "Mover Marketing Video",
									clientName: "Mover Marketing Video",
									code: token,
									...
									magicLinkUrl: url,
									noReplyMessage:
										"This sign-in email was sent by Mover Marketing Video.",
								},
							})
						) {
							return;
						...
						if (!serverEnv().RESEND_API_KEY) {
							...
							await sendEmail({
								email: identifier,
								subject: `Your Mover Marketing Video verification code`,
								react: email,
							});
						}
					},
