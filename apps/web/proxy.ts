...
const publicFileExtensionPattern =
	/\.(?:avif|bmp|css|gif|ico|jpg|jpeg|js|json|map|mjs|mp3|mp4|otf|png|svg|ttf|txt|wasm|webmanifest|webm|woff|woff2|xml)$/i;

export async function proxy(request: NextRequest) {
	const url = new URL(request.url);
	const path = url.pathname;

	if (publicFileExtensionPattern.test(path)) {
