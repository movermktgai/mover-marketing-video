		...
		return Effect.gen(function* () {
			...
			if (missingRequiredDuration || exceedsFreePlanLimit) {
				...
				if (!userIsPro(orgOwner)) {
					...
					return c.text(
						reportedDuration === null
							? "Recording duration is required to complete this upload."
							: "Recording exceeds the configured upload duration limit for this account. Contact the Mover Marketing AI team if this should be enabled.",
					);
				}
			}
