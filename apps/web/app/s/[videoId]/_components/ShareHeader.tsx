...
export const ShareHeader = ({
	...
	const handleHideBranding = async () => {
		...
		try {
			...
			toast.success("Product logo hidden");
			refresh();
		} catch (error) {
			toast.error(
				error instanceof Error ? error.message : "Failed to hide product logo",
			);
		} finally {
			...
			{userIsOwnerAndNotPro && (
				<div className="flex sticky flex-col sm:flex-row inset-x-0 top-0 z-10 gap-4 justify-center items-center px-3 py-2 mx-auto w-[calc(100%-20px)] max-w-fit rounded-b-xl border bg-gray-4 border-gray-6">
					<p className="text-center text-gray-12">
						Long shareable links are included for approved Mover Marketing Video
						accounts. Contact the Mover Marketing AI team if access is
						unavailable.
					</p>
				</div>
			)}
					...
					<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
						...
						{user !== null && (
							<div>
								...
								{userIsOwnerAndNotPro && (
									<p className="mt-2 mb-3 flex items-center text-sm text-gray-400">
										<Globe2 className="mr-1 w-4 h-4" />
										Custom domain access is unavailable for this account.
									</p>
								)}
							</div>
						)}
