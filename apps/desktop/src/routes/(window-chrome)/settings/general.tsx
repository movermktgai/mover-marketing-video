...
function AppearanceSection(props: {
	...
	return (
		<Section
			title="Appearance"
			description="Match Mover Marketing Video to your system theme or pick a fixed look."
		>
			<SectionCard padded>
				<div
...
function Inner(props: { initialStore: GeneralSettingsStore | null }) {
	...
	const managedWindowBundleIdentifiers = [
		"ai.movermarketing.video",
		"so.cap.desktop",
	];
	const managedWindowOwnerNames = ["mover marketing video", "cap"];
	const isManagedWindowsApp = (window: CaptureWindow) => {
		const bundleIdentifier = (window.bundle_identifier ?? "").toLowerCase();
		if (
			managedWindowBundleIdentifiers.some((id) => bundleIdentifier.includes(id))
		) {
			return true;
		}

		const ownerName = (window.owner_name ?? "").toLowerCase();
		return managedWindowOwnerNames.some((name) => ownerName.includes(name));
	};
		...
		>
			<SettingsPageContent>
				...
				{ostype === "macos" && (
					<Section
						title="App"
						description="Choose how Mover Marketing Video shows up on your system."
					>
						<SectionRows>
							<ToggleSettingItem
								label="Always show dock icon"
								description="Keep Mover Marketing Video in the dock even when no windows are open."
								value={!settings.hideDockIcon}
								onChange={(v) => handleChange("hideDockIcon", !v)}
							/>
							<ToggleSettingItem
								label="System notifications"
								description="Show notifications for clipboard copies, saved files, and more. You may need to allow Mover Marketing Video in your system's notification settings."
				...
				>
					<SectionRows>
						...
						<ToggleSettingItem
							...
							description="Mover Marketing Video removes the local file once it has uploaded successfully."
...
function StorageSection(props: {
	...
	return (
		<Section
			title="Storage"
			description="Where Mover Marketing Video saves your recordings."
		>
			<SectionCard padded>
		...
		<Section title="Privacy">
			<SectionRows>
				<ToggleSettingItem
					...
					description="Mover Marketing Video uses anonymous telemetry to improve reliability and fix bugs. We never collect recording contents, window titles, file paths, or personal information."
					value={props.value}
...
function UpdatesSection(props: {
	...
	return (
		<Section
			title="Updates"
			description="Choose which Mover Marketing Video builds you receive."
		>
			<SectionCard>
					...
					<div class="flex justify-between items-start gap-4">
						<div class="flex flex-col gap-0.5 min-w-0">
							...
							<p class="text-xs leading-snug text-gray-10">
								Which release channel Mover Marketing Video updates from.
							</p>
						</div>
						<SegmentedControl
							value={props.value}
		...
		toast.custom(
			(t) => (
				<div class="flex gap-3 items-center px-4 py-3 rounded-xl border shadow-lg bg-gray-1 border-gray-4 text-gray-12">
					<p class="text-sm">
						Higher Instant Mode resolutions are included for approved accounts.
						Contact the Mover Marketing AI team if this should be enabled.
					</p>
					<button
						type="button"
...
function CapProSection(props: {
	...
	return (
		<Section
			title="Pro"
			description="Settings available with an upgraded license."
			pro
		>
			<SectionRows>
...
function ServerURLSetting(props: {
	...
	return (
		<Section
			title="Self-host"
			description="Only change this if you are running your own Mover Marketing Video instance."
		>
			<SectionCard padded>
				<div class="flex flex-col gap-3">
					<label class="flex flex-col gap-1.5">
						<span class="text-[13px] text-gray-12">Server URL</span>
						<Input
							class="bg-gray-3"
							value={value()}
...
function ExcludedWindowsCard(props: {
	...
	return (
		<Section
			...
			description={
				props.isWindows
					? "Hide windows from recordings. On Windows, only app-related windows can be excluded."
						...
						<div class="flex items-start gap-2">
							...
							<div class="min-w-0 flex-1 space-y-1">
								<p class="text-xs font-medium text-amber-11">
									Recommended app windows are not excluded
								</p>
